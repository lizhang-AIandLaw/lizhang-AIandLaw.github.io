---
journeyPhases:
  - period: "Sep. 2015 - Jun. 2019"
    title: Bachelor of Electrical Engineering
    org: China University of Mining and Technology
    description: "Completed undergraduate training in **Electrical Engineering**, building a strong analytical and quantitative foundation. Graduated with a **90.91/100 GPA** and ranked **2/278**, which shaped an early interest in rigorous technical problem-solving."
    tags: [Electrical Engineering, Quantitative Methods, Top Ranking]
  - period: "Jul. 2017 - Dec. 2017"
    title: Study Abroad in Engineering and Computer Science
    org: Australian National University
    description: "Studied abroad at the **Australian National University** in the College of Engineering and Computer Science, broadening exposure to interdisciplinary computing and engineering methods while earning a **6.5/7.0 GPA**."
    tags: [Study Abroad, Engineering and CS, International Experience]
  - period: "Sep. 2019 - Jun. 2022"
    title: Master of Computational Law
    org: Tsinghua University
    description: "Completed graduate study in **Computational Law** at **Tsinghua University**, where legal reasoning, computation, and interdisciplinary research became the core of my academic work."
    tags: [Legal Theory, Computational Law, Interdisciplinary Research]
  - period: "Jun. 2022 - Jun. 2024"
    title: Legal Counselor
    org: The Export-Import Bank of China, Nanjing, China
    description: "Served as **Legal Counselor** at **The Export-Import Bank of China** in Nanjing, spearheading legal negotiations for complex banking transactions and ensuring strict regulatory compliance. Drafted and reviewed credit facility agreements and security documents for cross-border financing, while advising internal stakeholders on operational risk mitigation and dispute resolution."
    tags: [Banking Law, Cross-border Financing, Regulatory Compliance, Risk Mitigation]
  - period: "Aug. 2024 - Present"
    title: Ph.D. Student in Intelligent Systems
    org: University of Pittsburgh
    description: "Currently pursuing a **Doctor of Philosophy in Intelligent Systems** at the **University of Pittsburgh**, advised by **Prof. Kevin Ashley**. My research focuses on **AI and Law**, especially legal reasoning with large language models, trustworthy multi-agent systems, and evaluation of hierarchical legal reasoning."
    tags: [Intelligent Systems, AI and Law, Reasoning LLMs, Trustworthy AI]
mentorship:
  heading: Collaborators
  description: ""
  mentees: []
version:
  current: v1.0.0
  history:
    - version: v1.0.0
      title: Initial Portfolio
      features:
        - Personal bio and journey timeline
        - Research publications section
        - News and updates
---

I am a Ph.D. student in **Intelligent Systems** at the [University of Pittsburgh](https://www.pitt.edu/), advised by [Prof. Kevin Ashley](https://www.law.pitt.edu/people/kevin-d-ashley). Before starting my doctoral work in August 2024, I worked as a Legal Counselor at [The Export-Import Bank of China](https://www.eximbank.gov.cn/), where I focused on banking transactions, cross-border financing, regulatory compliance, and dispute resolution. Previously, I completed a **Master of Computational Law** at [Tsinghua University](https://www.tsinghua.edu.cn/en/index.htm), studied abroad at the [Australian National University](https://www.anu.edu.au/), and earned a **Bachelor of Electrical Engineering** from [China University of Mining and Technology](https://global.cumt.edu.cn/).

My research explores the intersection of **AI** and **Law**, where I design intelligent systems with dual purpose: (1) for **legal practitioners**, I build computational tools that leverage large language models to support legal reasoning, case analysis, and document review; and (2) for broader users, I design systems that make complex legal knowledge more accessible, reliable, and actionable.

My research interests include **Reasoning LLMs**, **hierarchical legal reasoning**, **trustworthy intelligent systems**, and **AI for Law**.
